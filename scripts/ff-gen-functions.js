...
function initAcctTypes(){
    dropDownSelections("#sel-acct-type", omnicchannelDb.applications[$("#sel-application").val()].acctTypes);
...
function initFunc(){
    ...
    dropDownSelections("#sel-func", omnicchannelDb.applications[$("#sel-application").val()].functionalities);
...
function addFunc(){
    $("#sel-func").on("change", function(event){
        target = event.target;
        funcSel(target.value, omnicchannelDb.applications[scenario.application].functionalities);
    });
...
function doneFunc(){
    if(!standAloneTradeValid()){
        ...
        if(c){
            ...
            if(funcArrayComp(omnicchannelDb.standaloneFunc) || (scenario.func.length == 1 && scenario.func[0] == "Return w/ Exchange")){
...
function eligPmtType(){
    ...
    if(omnicchannelDb.applications[scenario.application].pmtTypes.length){
        if(!isNoPmtElig()){
...
function isNoPmtElig(){
    ...
    if(scenario.numAcces != null){
        ...
        for(i=1; i<=scenario.numNewAal; i++){
            if(!omnicchannelDb.nonPayDevTypes.includes(scenario.lineLvlType[i].devTypeNewAal)){
                return false;
            }
        }
        for(i=1; i<=scenario.numUpg; i++){
            if(!omnicchannelDb.nonPayDevTypes.includes(scenario.lineLvlType[i].devTypeUpg)){
                return false;
            }
        }
        for(i=1; i<=scenario.numRetEx; i++){
            if(!omnicchannelDb.nonPayDevTypes.includes(scenario.devTypeRetEx)){
                return false;
            }
        }
...
function initTypeDtls(){
    ...
    let pmtTypes = omnicchannelDb.applications[scenario.application].pmtTypes;
    if(!funcArrayComp(deplPmtTypeFuncs)){
        ...
        dropDownSelections("#depl-type-sel", omnicchannelDb.applications[scenario.application].deplTypes);
        if(!pmtTypes.length){
    ...
    if(!funcArrayComp(devContrTypeFuncs)){
        ...
        if(scenario.func.includes("Return w/ Exchange")){
            ...
            dropDownSelections("#dev-type-retEx-sel", omnicchannelDb.applications[scenario.application].devTypes);
            dropDownSelections("#contr-type-retEx-sel", omnicchannelDb.contrTypes);
            ...
            if(scenario.func.includes("New Activation") || scenario.func.includes("Add-A-Line")){
                ...
                for(let i=1; i<=n; i++){
                    dropDownSelections("#dev-type-" + `${i}` + "-sel", omnicchannelDb.applications[scenario.application].devTypes);
                    dropDownSelections("#contr-type-" + `${i}` + "-sel", omnicchannelDb.contrTypes);
    ...
    if(!omnicchannelDb.applications[scenario.application].tradeTypes.length){
        ...
        for(let i=1; i<=scenario.numTradeIn; i++){
            dropDownSelections("#trade-type-" + `${i}` + "-sel", omnicchannelDb.applications[scenario.application].tradeTypes);
    ...
    if(scenario.func.includes("New Activation")){
        $("#plan-type").show();
        dropDownSelections("#plan-type-sel", omnicchannelDb.planTypes);
        ...
        let aalUpgPlanOpts = [omnicchannelDb.planTypes[0], omnicchannelDb.planTypes[4]];
        $("#plan-type").show();
...
function addDvcContrTypeDtls(ele, key){
    ...
    if(omnicchannelDb.nonPayDevTypes.includes(ele.value)){
