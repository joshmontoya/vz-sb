...
function dispFunc(){
    ...
    for(i = 0; i < scenario.func.length; i++){
        if(i == scenario.func.length - 1){
            e += scenario.func[i];
        }else{
            e += scenario.func[i] + ", ";
        }  
    }
    $(".container-func-sel").css("visibility", "visible");
    $("#funcSel-res").html(e);
    $(".container-func-conf").css("visibility", "visible");
}
...
function doneFunc(){
    if(!scenario.func.length == 0){
        ...
        if(c){
            $("#selFunc").attr("disabled", "disabled");
            $(".container-func-conf").css("display", "none");
            $(".container-3-3").css("visibility", "visible");
        }else{
            c.preventDefault();
        }
    }else{
...
const dataObj = 
{
    ...
    "acctTypes" : ["PE", "PE w/ ECPD", "BE w/o ECPD", "ME", "SL", "BE"],
